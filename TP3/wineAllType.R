...
# Formula to calculate FatigueLevel
  formula_B <- quality ~ fixed_acidity + volatile_acidity + citric_acid + chlorides + free_sulfur_dioxide + total_sulfur_dioxide + pH + sulphates + alcohol + type
  ...
  # Define input and test variables
  test_B <- subset(test, select = c("fixed_acidity", "volatile_acidity", "citric_acid", "chlorides","free_sulfur_dioxide","total_sulfur_dioxide","pH","sulphates","alcohol","type"))
  ...
  # Calculate RMSE
  rmse(c(test$quality), c(results_Bprediction))
...
# --------------
# Teste C -> Without residual sugar, density and chlorides
# --------------

# Formula to calculate FatigueLevel
  formula_C <- quality ~ fixed_acidity + volatile_acidity + citric_acid + free_sulfur_dioxide + total_sulfur_dioxide + pH + sulphates + alcohol + type
  ...
  # Define input and test variables
  test_C <- subset(test, select = c("fixed_acidity", "volatile_acidity", "citric_acid","free_sulfur_dioxide","total_sulfur_dioxide","pH","sulphates","alcohol"))
  
  # Test the neural net with new cases
  rna_C.results <- compute(rna_C, test_C)
  
  # Print the results
  results_C <- data.frame(current = test$quality, prediction = rna_B.results$net.result)
