...
# Read file to memory
data <- read.csv("~/Desktop/SRCR/Trabalhos/Local/TP3/Dados/winequality-red-formatado.csv", header=TRUE, sep=";", dec=".")
...
# --------------
#	Teste B	-> Without chlorides and density
# --------------

# Formula to calculate FatigueLevel
  formula_B <- quality ~ fixed_acidity + volatile_acidity + citric_acid + residual_sugar + free_sulfur_dioxide + total_sulfur_dioxide + pH + sulphates + alcohol
  ...
  # Define input and test variables
  test_B <- subset(test, select = c("fixed_acidity", "volatile_acidity", "citric_acid", "residual_sugar","free_sulfur_dioxide","total_sulfur_dioxide","pH","sulphates","alcohol"))
